...
function findUserByRole (client) {
  return function findUserByRole (realmName, id, roleName) {
    return new Promise((resolve, reject) => {
      const req = {
        auth: {
          bearer: privates.get(client).accessToken
      ...
      if (roleName) {
        req.url = `${client.baseUrl}/admin/realms/${realmName}/roles/${roleName}/users`;
      } else {
        req.url = `${client.baseUrl}/admin/realms/${realmName}/clients/${id}/roles`;
      }
